...
import type { CtaContent } from "@content/homepage/types"
...
const CtaSection = ({ content }: { content: CtaContent }) => (
  <section style={{ background: "#EB630D" }} className="py-16 lg:py-24 px-5 md:px-6">
    <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
      <span style={{ display: "inline-block", fontFamily: "var(--font-heading)", fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.12em", color: "#fff", background: "rgba(255,255,255,0.25)", padding: "5px 14px", borderRadius: 99, marginBottom: 20 }}>{content.tag}</span>
      <h2 style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(26px, 3.8vw, 44px)", fontWeight: 800, color: "#fff", letterSpacing: "-0.02em", lineHeight: 1.15, marginBottom: 16, marginTop: 0 }}>{content.heading}</h2>
      <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 16, lineHeight: 1.7, marginBottom: 36 }}>{content.body}</p>
